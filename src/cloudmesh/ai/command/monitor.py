...
console = Console()
logger = get_logger("cloudmesh.ai.monitor.cli")

def get_gui_framework():
    return os.getenv("CMC_GUI", "terminalgui")

# Create the CLI group at the module level
@click.group()
def monitor_group():
    """cloudmesh-ai-monitor: Observability for AI infrastructure."""
    pass

# Define the groups and commands on monitor_group
@monitor_group.group(name="setup")
def setup_group():
...
def setup_add(hostname, label):
    ...
    logger.info(f"Added host {hostname} to configuration.")
...
def setup_remove(hostname):
    ...
    logger.info(f"Removed host {hostname} from configuration.")
...
def setup_activate(hostname):
    ...
    logger.info(f"Host {hostname} activated.")
...
def setup_deactivate(hostname):
    ...
    logger.info(f"Host {hostname} deactivated.")
    console.print(f"[yellow]Host {hostname} deactivated.[/yellow]")

@monitor_group.command(name="run")
...
def stats_cmd(stat_type, identifiers):
    ...
    logger.info(f"Checking {stat_type} stats for {len(identifiers)} hosts.")
    hm = HostManager()
...
@monitor_group.command(name="probe")
...
def probe_cmd(hosts, config):
    ...
    from cloudmesh.ai.common.io import load_yaml, dump_yaml, path_expand
    
    config_path = path_expand(config)
...
@monitor_group.command(name="dashboard")
...
def check_exporter_cmd(url):
    ...
    import requests
    logger.info(f"Checking GPU Exporter at {url}...")
...
@monitor_group.command(name="oom-check")
def oom_check_cmd():
    """Scan system logs for Out-of-Memory (OOM) events."""
    import subprocess
    from pathlib import Path
    logger.info("Scanning for OOM events...")
...
@monitor_group.command(name="grafana-snapshot")
...
@monitor_group.group(name="health")
def health_group():
    """Check the health status of various system components."""
    pass

@monitor_group.group(name="status")
def status_group():
...
@monitor_group.command(name="llm-check")
...
def register(cli):
    """Register the monitor CLI group with the main CMC CLI."""
    import sys
    print("DEBUG: Implementation register called", file=sys.stderr)
